...
- a **`HelmRelease`** named `voicemail` that renders a chart from the same repo and installs it, ordered after `apps` with `dependsOn`
...
4. **HelmRelease and dependencies** — read the `voicemail` release and how `dependsOn` ordered it after `apps`.
