...
# --- 4. apply the Flux custom resources (HEALTHY full pipeline) ---------------
# GitRepository (source) -> Kustomization `apps` (builds ./apps) + two
# HelmReleases: `message-store` (the voicemail backing store) and `voicemail`
# (renders ./charts/voicemail), ordered after message-store via dependsOn.
cat <<'EOF' | kubectl apply -f - >/dev/null
  ...
  chart:
    spec:
      ...
      sourceRef:
        kind: GitRepository
        name: polyphone-config
  # voicemail waits for its backing store's HelmRelease before installing.
  # dependsOn references other HelmReleases (same kind), not Kustomizations.
  dependsOn:
    - name: message-store
  values:
    replicaCount: 2
...
# Force the first reconcile so the tour cluster is ready without waiting out the
# interval. Reconcile the dependency (message-store) before voicemail, then wait
# for the GitOps-managed workloads to come up.
...
flux reconcile helmrelease voicemail --timeout=3m >/dev/null 2>&1
