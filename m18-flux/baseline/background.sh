...
# GitRepository (source) -> Kustomization `apps` (builds ./apps) -> HelmRelease
# `voicemail` (renders ./charts/voicemail, ordered after `apps` via dependsOn).
cat <<'EOF' | kubectl apply -f - >/dev/null
...
# interval, then wait for the GitOps-managed workloads to come up.
