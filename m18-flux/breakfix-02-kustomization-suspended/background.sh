...
# --- 4. apply the Flux custom resources (with a drifted + suspended app) ------
# The incident story: last night an engineer scaled `dialplan` to 5 by hand and
# SUSPENDED the `apps` Kustomization so Flux wouldn't revert the emergency
# change. The incident is over, but the suspend was never lifted — so dialplan
# is stuck at 5 (git declares 2) and every future change to `apps` is ignored.

# The hand-scaled, drifted live Deployment (5 replicas). Applied directly, NOT
# through Flux — this is the out-of-band change Flux would normally correct.
cat <<'EOF' | kubectl apply -f - >/dev/null
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  labels: { app: dialplan, plane: control, tier: lab }
spec:
  replicas: 5                         # drift: git declares 2 (see ./apps/dialplan.yaml)
  selector: { matchLabels: { app: dialplan } }
  template:
    metadata:
      labels: { app: dialplan, plane: control, tier: lab }
...
spec:
  ...
  # >>> breakfix-02 mutation: the Kustomization is suspended, so kustomize-
  #     controller does not reconcile it — the drifted dialplan (5 replicas) is
  #     never pulled back to the git-declared 2, and no new commit would apply.
  suspend: true                       # MUTATED (baseline: false / omitted)
  # <<< breakfix-02 mutation ends
EOF

# Make the source Ready so the only thing wrong is the suspend (not the fetch).
flux reconcile source git polyphone-config --timeout=90s >/dev/null 2>&1
# <<< M18 addition ends
