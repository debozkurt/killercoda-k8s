...
#
# Same three-layer fleet as the baseline (base -> regions -> clusters), written
# to /root/fleet. In THIS scenario the us-east-1 region raised its session-capacity
# standard to 8000, but the prod-us-east-1 leaf carries a leftover per-cluster
# MAX_SESSIONS=5000 override from an earlier plan. The cluster overlay sits later
# in the composition stack, so 5000 wins — SHADOWING the new regional 8000. See the
# fenced breakfix-02 mutation below.
# ===========================================================================
...
spec: { replicas: 3 }
EOF
# >>> breakfix-02 mutation: prod-us-east-1 carries a leftover per-cluster
#     MAX_SESSIONS=5000 override from an earlier capacity plan. The us-east-1 region
#     now sets the standard to 8000 (regions/us-east-1), but the cluster overlay sits
#     LATER in the composition stack (base -> region -> cluster), so 5000 wins. The
#     cluster renders, applies, and runs on 5000 — silently shadowing the new regional
#     8000. The owning layer (region) is correct; a more-specific layer is SHADOWING
#     it. Fix = delete this per-cluster configMapGenerator override so the region's
#     8000 flows through (or reconcile it to 8000).
...
patches:
  - path: replicas-patch.yaml
# Leftover per-cluster capacity override from before the standard moved to the region layer.
configMapGenerator:
  - name: edge-relay-config
    behavior: merge
    literals:
      - MAX_SESSIONS=5000   # MUTATED (baseline: absent — prod inherits the region's 8000; this leftover shadows it)
EOF
...
# --- apply prod-us-east-1. It comes up HEALTHY on 5000 — the render is valid and
#     the workload runs; the value is just shadowing the new regional 8000.
