...
# to /root/fleet. In THIS scenario the eu-central-1 region overlay was cloned
# from us-east-1 and its REGION cluster variable was never updated. Every cluster
# in eu-central-1 (here prod-eu-central-1) renders, applies, and RUNS reporting
# the wrong region — the value is STALE IN ITS OWNING LAYER. See the fenced
# breakfix-01 mutation below.
# ===========================================================================
...
# >>> breakfix-01 mutation: regions/eu-central-1 was cloned from regions/us-east-1.
#     The clone updated the `region:` label but left the REGION config literal reading
#     us-east-1. The region overlay OWNS REGION, so every cluster in eu-central-1
#     inherits the stale value: prod-eu-central-1 renders, applies, and runs fine —
#     it just reports REGION=us-east-1. The render is valid; nothing is CrashLooping;
#     only the value is wrong, and only for this region. This is a value STALE IN ITS
#     OWNING LAYER. Fix = set REGION=eu-central-1 here.
...
configMapGenerator:
  - name: edge-relay-config
    behavior: merge
    literals:
      - REGION=us-east-1        # MUTATED (baseline: REGION=eu-central-1 — this region's own identity)
      - MAX_SESSIONS=4000
EOF
...
# --- apply the eu-central-1 prod cluster. It comes up HEALTHY (the render is
#     valid and the workload runs) — it just carries the wrong REGION. The learner
#     inspects it and traces the value back to the region overlay.
kubectl apply -k /root/fleet/clusters/prod-eu-central-1 >/dev/null 2>&1
