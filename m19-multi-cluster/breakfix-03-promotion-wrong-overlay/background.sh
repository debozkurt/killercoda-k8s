...
# to /root/fleet. In THIS scenario the promotion of nginx:1.27 to STAGE was
# botched: the pin was added to the prod leaf instead of the stage leaf. prod
# overshot the gate to 1.27; stage never advanced and still renders 1.25. The
# right value landed in the WRONG LAYER. See the two fenced breakfix-03 mutations.
# ===========================================================================
...
spec: { replicas: 2 }
EOF
# >>> breakfix-03 mutation: STAGE leaf — the promotion of nginx:1.27 to stage was
#     botched; the pin was added to the prod leaf (below) instead of here, so stage
#     never advanced and still renders 1.25. Fix = set this newTag back to "1.27".
...
images:
  - name: nginx
    newTag: "1.25"        # MUTATED (baseline: "1.27" — the tag being promoted to stage)
patches:
...
spec: { replicas: 3 }
EOF
# >>> breakfix-03 mutation: PROD leaf — the 1.27 pin meant for stage landed here.
#     Baseline prod-us-east-1 has NO image pin (it inherits the base 1.25); this added
#     images: block makes prod overshoot the promotion gate to 1.27. Fix = remove this
#     images: block so prod goes back to the inherited 1.25.
...
images:
  - name: nginx
    newTag: "1.27"        # MUTATED (baseline: prod has NO images pin — it inherits the base default 1.25)
patches:
  - path: replicas-patch.yaml
EOF
# <<< breakfix-03 mutation ends
...
# --- apply the STAGE cluster — the tier that should have advanced but didn't.
#     It comes up HEALTHY on the stale 1.25. prod-us-east-1 is left rendered-only so
#     the learner can see it overshot to 1.27 without applying it (no collision).
kubectl apply -k /root/fleet/clusters/stage-us-east-1 >/dev/null 2>&1
