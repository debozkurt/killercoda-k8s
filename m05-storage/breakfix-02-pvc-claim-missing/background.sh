...
# >>> breakfix-02 mutation: the directory Deployment mounts a claim named
#     "directory-store", but no such PVC exists — the real claim is "directory-data"
#     (created just below, and it binds fine). The Pod references a PVC that is not
#     in the namespace, so it stays Pending: `describe pod` says
#     `persistentvolumeclaim "directory-store" not found`. Distinct from breakfix-01:
#     the claim is ABSENT, not Pending. Fix = point claimName at the claim that exists.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
      ...
      volumes:
        - name: data
          persistentVolumeClaim:
            claimName: directory-store   # MUTATED (baseline: directory-data — directory-store does not exist)
---
apiVersion: v1
kind: Service
metadata:
