...
# >>> breakfix-01 mutation: cdr-data claims StorageClass "fast-ssd", which does not
#     exist on this cluster. With no such class there is no provisioner to call, so no
#     PV is ever created and the PVC sits in Pending forever. cdr-writer, which mounts
#     cdr-data, cannot start — it stays Pending ("pod has unbound PersistentVolumeClaims").
#     Nothing crashes; the workload just waits on a volume that never arrives. Fix =
#     point storageClassName at the real class (local-path). storageClassName is
#     immutable once set, so the fix is delete + recreate the PVC, not edit.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
...
spec:
  accessModes: [ReadWriteOnce]
  storageClassName: fast-ssd   # MUTATED (baseline: local-path — fast-ssd is not installed on this cluster)
