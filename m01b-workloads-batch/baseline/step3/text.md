...
Two lines near the top spell out the sizing:
...
`COMPLETIONS 4/4` — four succeeded out of four required. Correctness here lives in whether that `4` target matches the real number of shards — not in the word `Complete`. That exact failure is `breakfix-03`.
...
`COMPLETIONS 4/4` — all four shards exported. A parallel Job, sized and complete.
