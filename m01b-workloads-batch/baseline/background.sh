...
# ===========================================================================
# batch plane  (M01b enhancement — Jobs & CronJobs layered onto the fleet)
# >>> M01b batch enhancement: three run-to-completion / scheduled workloads the
#     standard 17-workload fleet doesn't carry. schema-migrate (one-shot Job),
#     usage-export (parallel Job — completions/parallelism), cdr-rollup
#     (CronJob). Batch pods must EXIT to count as done, so they run busybox with
#     an explicit command rather than the long-lived nginx the rest of the fleet
#     uses (nginx never returns, so it could never "complete"). This is the
#     "what good looks like" batch reference the M01b breakfixes each mutate.
# ===========================================================================

# schema-migrate — one-shot Job: run-to-completion schema migration.
# restartPolicy OnFailure (Jobs forbid Always); backoffLimit caps retries.
cat <<'EOF' | kubectl apply -f -
apiVersion: batch/v1
kind: Job
metadata:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: migrate
          image: busybox:1.36
          command: ["/bin/sh","-c","echo '[schema-migrate] connecting to postgres.polyphone.example'; echo '[schema-migrate] applying 001_init'; echo '[schema-migrate] applying 002_indexes'; sleep 5; echo '[schema-migrate] done'"]
          resources:
            requests: { cpu: 25m, memory: 32Mi }
            limits:   { cpu: 100m, memory: 64Mi }
EOF

# usage-export — parallel Job: 4 daily shards, 2 at a time.
# completions=4 (total successes needed), parallelism=2 (max concurrent pods).
cat <<'EOF' | kubectl apply -f -
apiVersion: batch/v1
kind: Job
metadata:
...
# cdr-rollup — CronJob: roll up Call Detail Records every minute (lab cadence;
# real cadence would be hourly/nightly). Forbid overlap; keep a little history.
cat <<'EOF' | kubectl apply -f -
apiVersion: batch/v1
kind: CronJob
metadata:
