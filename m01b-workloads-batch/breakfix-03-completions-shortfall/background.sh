...
# >>> breakfix-03 mutation: usage-export should process 4 daily shards
#     (completions: 4), but completions was hand-edited down to 1. The Job runs
#     ONE pod, reports COMPLETIONS 1/1 and Complete — it looks perfectly healthy
#     — while 3 of the 4 usage shards are silently never exported. Teaching:
#     completions/parallelism semantics, and that a green/Complete Job can still
#     be WRONG (the batch analog of "Running ≠ Ready" from M01). Fix = delete &
#     recreate with completions: 4, because the Job spec is immutable.
cat <<'EOF' | kubectl apply -f -
apiVersion: batch/v1
kind: Job
metadata:
...
spec:
  completions: 1                # MUTATED (baseline: completions: 4 — one pod per daily shard)
  parallelism: 2
  backoffLimit: 4
  ttlSecondsAfterFinished: 3600
  template:
