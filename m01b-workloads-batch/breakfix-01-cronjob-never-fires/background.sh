...
# usage-export — parallel Job (healthy)
cat <<'EOF' | kubectl apply -f -
apiVersion: batch/v1
kind: Job
metadata:
...
# >>> breakfix-01 mutation: the cdr-rollup CronJob is suspended. A suspended
#     CronJob is syntactically valid and looks healthy — it simply never creates
#     Jobs. No pods, no history, LAST SCHEDULE <none>, ACTIVE 0, and the tell:
#     SUSPEND True. The nightly CDR rollup silently stops and downstream
#     billing/analytics quietly starve of data. Teaching: a CronJob that "does
#     nothing" is almost always suspended or carries a schedule that never
#     matches — read the spec, don't assume the controller is broken.
cat <<'EOF' | kubectl apply -f -
apiVersion: batch/v1
kind: CronJob
metadata:
...
spec:
  schedule: "* * * * *"
  suspend: true                 # MUTATED (baseline: suspend omitted/false → fires every minute)
