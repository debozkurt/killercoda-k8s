...
# >>> breakfix-02 mutation: the schema-migrate Job's command has a typo on its
...
#     cannot patch the command in place.
cat <<'EOF' | kubectl apply -f -
apiVersion: batch/v1
kind: Job
metadata:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: migrate
          image: busybox:1.36
          command: ["/bin/sh","-c","echo '[schema-migrate] connecting to postgres.polyphone.example'; echo '[schema-migrate] applying 001_init'; sleep 3; ecaho '[schema-migrate] done'"]   # MUTATED (baseline: final step is `echo`, not `ecaho` → exits 0)
          resources:
            requests: { cpu: 25m, memory: 32Mi }
            limits:   { cpu: 100m, memory: 64Mi }
EOF
