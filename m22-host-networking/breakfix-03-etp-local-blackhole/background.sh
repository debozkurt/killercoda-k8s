...
# ===========================================================================
# breakfix-03 — NodePort with externalTrafficPolicy: Local and no per-node endpoint
# ===========================================================================

# >>> breakfix-03 mutation: rtp-ingress NodePort set to externalTrafficPolicy: Local,
# but its single backing Pod lands on ONE node. `Local` forwards only to node-local
# endpoints and DROPS traffic on any node that has none. So the NodePort answers on the
# node running the Pod and blackholes on every other node — while `get svc` looks normal
# and the EndpointSlice is fully populated. Fix: externalTrafficPolicy: Cluster (or put
# an endpoint on every node, e.g. a DaemonSet).
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  type: NodePort
  externalTrafficPolicy: Local   # MUTATED (baseline: Cluster) — drops on nodes with no local endpoint
...
# The Pod comes up fine (reachability is what broke), so waiting for it is safe.
