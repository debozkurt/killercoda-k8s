...
# ===========================================================================
# breakfix-01 — hostNetwork Pod that lost cluster DNS
# ===========================================================================

# >>> breakfix-01 mutation: rtp-relay on hostNetwork WITHOUT ClusterFirstWithHostNet.
# A hostNetwork Pod left at the default dnsPolicy (ClusterFirst) does NOT get cluster
# DNS — the kubelet hands it the NODE's /etc/resolv.conf instead of CoreDNS. So it
# cannot resolve any <svc>.<ns>.svc.cluster.local name. The Pod is Running; only name
# resolution to in-cluster Services is broken. Fix: dnsPolicy: ClusterFirstWithHostNet.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      hostNetwork: true
      dnsPolicy: ClusterFirst   # MUTATED (baseline: ClusterFirstWithHostNet) — no cluster DNS on host netns
      affinity:
        nodeAffinity:
          requiredDuringSchedulingIgnoredDuringExecution:
            nodeSelectorTerms:
...
# The Pod itself comes up fine (DNS is what broke), so waiting for it is safe.
