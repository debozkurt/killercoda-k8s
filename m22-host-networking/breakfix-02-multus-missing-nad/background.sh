...
# ===========================================================================
# breakfix-02 — multi-NIC Pod whose NetworkAttachmentDefinition isn't where it looks
# ===========================================================================
...
# The macvlan NAD exists — but only in the `media` namespace.
cat <<'EOF' | kubectl apply -f -
...
# >>> breakfix-02 mutation: media-probe rolled out in the `edge` namespace, asking for
# the `rtp-macvlan` network by BARE name. NADs are namespaced, and a bare network name
# is resolved against the POD's namespace — Multus looks for `rtp-macvlan` in `edge`,
# where it does not exist. The Pod hangs in ContainerCreating: sandbox setup fails
# because the requested NAD can't be found. Fix: reference it cross-namespace as
# `media/rtp-macvlan` (or create the NAD in `edge`).
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  name: media-probe
  namespace: edge
  labels: { app: media-probe, plane: edge, tier: lab }
spec:
  replicas: 1
  ...
  template:
    metadata:
      labels: { app: media-probe, plane: edge, tier: lab }
      annotations:
        k8s.v1.cni.cncf.io/networks: rtp-macvlan   # MUTATED (correct: media/rtp-macvlan) — this pod is in edge; the NAD is in media, and a bare name only resolves in the pod's own ns
    spec:
      affinity:
        nodeAffinity:
          requiredDuringSchedulingIgnoredDuringExecution:
...
# media-probe is intentionally broken (stuck ContainerCreating) — do NOT wait on it,
# that would just burn the timeout. Wait for Multus and the fleet.
kubectl rollout status daemonset/kube-multus-ds -n kube-system --timeout=120s >/dev/null 2>&1

# Fleet-settle wait kept LAST so nothing critical sits behind it.
