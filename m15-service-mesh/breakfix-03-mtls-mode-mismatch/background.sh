...
# only changes the DestinationRule tls mode (all pods stay 2/2 and Ready), so every
...
# >>> breakfix-03 mutation: the client-side TLS mode is DISABLE, but the server side
#     (PeerAuthentication default) is STRICT. So every caller's sidecar sends PLAINTEXT to
#     session-broker, whose sidecar accepts only mTLS and resets the connection — callers
#     get a 503. Both ends are 2/2 and in the mesh; the Service has endpoints; the subset is
#     correct. The two mTLS settings simply disagree — a "half-configured" mesh. Automatic
#     mTLS would have handled this; the explicit DISABLE override is what breaks it.
#     Fix = set the DestinationRule tls mode to ISTIO_MUTUAL so both sides speak mTLS.
cat <<'EOF' | kubectl apply -f -
...
spec:
  ...
  trafficPolicy:
    tls:
      mode: DISABLE          # MUTATED (baseline: ISTIO_MUTUAL) — plaintext into a STRICT server → 503
    connectionPool:
