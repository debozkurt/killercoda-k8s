...
# Shorter ceilings — Killercoda caps background-script runtime. The mutation breaks image
# admission for one workload, not Pod health, so every fleet Pod still becomes Ready.
...
# >>> breakfix-03 mutation: deploy call-recorder pinned to nginx:latest. disallow-latest-tag
#     (Enforce, autogen disabled) admits the Deployment but rejects the Pod its ReplicaSet
#     creates, so call-recorder sits at 0/1 with no Pods and the denial on a FailedCreate
#     event. Limits ARE set, so require-resource-limits passes — the only violation is the
#     :latest tag. Fix = pin the image to an explicit non-latest tag (nginx:1.25).
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  labels: { app: call-recorder, plane: tenant, tier: lab }
spec:
  replicas: 1
  ...
  template:
    ...
    spec:
      containers:
        - name: app
          image: nginx:latest          # MUTATED (baseline: nginx:1.25) — :latest violates disallow-latest-tag
          ports: [{ containerPort: 80 }]
          resources:
            requests: { cpu: 25m, memory: 32Mi }
            limits:   { cpu: 100m, memory: 64Mi }
