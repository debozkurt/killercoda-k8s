...
# >>> M20 setup: Kyverno healthy policy set + one compliant governed workload (the tour).
...
kubectl wait --for=condition=Available deployment/kyverno-admission-controller \
  -n kyverno --timeout=180s >/dev/null 2>&1
...
# tenant-web — a compliant tenant workload: limits set (passes require-resource-limits),
# nginx:1.25 tagged (passes disallow-latest-tag); add-owner-label injects owner=platform.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
