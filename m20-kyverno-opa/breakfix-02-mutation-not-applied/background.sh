...
# mutate policy's match, not Pod health, so every fleet Pod still becomes Ready.
...
# >>> breakfix-02 mutation: re-apply add-owner-label with a typo'd namespace, then deploy
#     tenant-portal. The mutate rule now matches namespace "tenant-app" (which doesn't exist),
#     so it never selects tenant-portal's Pods and the owner=platform label is never injected.
#     tenant-portal is otherwise compliant (limits set, nginx:1.25), so it runs fine — the
#     ONLY symptom is the missing label. Fix = correct the namespace back to tenant-apps, then
#     rollout restart tenant-portal so its Pods are re-admitted (mutation happens at admission).
cat <<'EOF' | kubectl apply -f -
apiVersion: kyverno.io/v1
kind: ClusterPolicy
metadata:
    ...
    - name: add-owner
      match:
        any:
          - resources:
              kinds: [Pod]
              namespaces: [tenant-app]     # MUTATED (baseline: tenant-apps) — typo matches no namespace
      mutate:
        patchStrategicMerge:
          metadata:
            labels:
              +(owner): platform
EOF
kubectl wait --for=condition=Ready clusterpolicy/add-owner-label --timeout=60s >/dev/null 2>&1
sleep 5
...
metadata:
  ...
  labels: { app: tenant-portal, plane: tenant, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: tenant-portal } }
  template:
    metadata:
      labels: { app: tenant-portal, plane: tenant, tier: lab }
    spec:
      containers:
        - name: app
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
EOF
kubectl wait --for=condition=Available deployment/tenant-portal -n tenant-apps --timeout=90s >/dev/null 2>&1
