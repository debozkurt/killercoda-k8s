...
# M03 config setup: app-config (LOG_LEVEL=info) consumed by session-broker as
# env. Healthy at boot; the breakfix-03 mutation edits it AFTER the pod is
# running (see the end of this script) so the env goes stale.
...
# M03 config setup: consume app-config as env (envFrom). The pod bakes
# LOG_LEVEL=info into its environment at start; the later CM edit won't reach it.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
if [ "$CMVAL" != "debug" ]; then
  echo "FATAL: app-config LOG_LEVEL edit did not take (got '$CMVAL', expected 'debug')." >&2
  echo "       The stale-env scenario depends on the ConfigMap and the pod disagreeing." >&2
  exit 1
fi
