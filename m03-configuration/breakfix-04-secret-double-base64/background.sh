...
# >>> breakfix-04 mutation: database-creds with a DOUBLE base64-encoded password.
#     The intended secret is DB_PASSWORD=changeme. Correct data would be
#     base64("changeme") = Y2hhbmdlbWU=. Here data holds base64("Y2hhbmdlbWU=") =
#     WTJoaGJtZGxiV1U9 — encoded one time too many. The kubelet decodes once, so
#     the container's env DB_PASSWORD becomes the literal string "Y2hhbmdlbWU="
#     (still base64), not "changeme". The pod runs fine; the credential is wrong.
#     Models the classic hand-base64 mistake — encoding a value that was already
#     base64, or pasting an encoded value into `data` a second time.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: Secret
metadata:
...
data:
  ...
  DB_PASSWORD: WTJoaGJtZGxiV1U9      # MUTATED (baseline: Y2hhbmdlbWU= = base64 "changeme"); this is base64 of THAT
EOF
