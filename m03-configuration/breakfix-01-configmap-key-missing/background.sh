...
# >>> breakfix-01 mutation: app-config has LOG_LEVEL and MAX_SESSIONS — but NOT
#     MAX_CONNECTIONS, which session-broker's env reference (below) asks for. A
#     required configMapKeyRef to a key that doesn't exist makes the kubelet
#     fail container creation with CreateContainerConfigError. Models a manifest
#     that referenced a config key under a name that was never added to the map.
...
# >>> breakfix-01 mutation: a required env reference to a key that doesn't exist
#     in app-config (MAX_CONNECTIONS). The pod fails at CreateContainerConfigError.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
          ...
          env:
            - name: MAX_SESSIONS
              valueFrom:
                configMapKeyRef:
                  name: app-config
                  key: MAX_CONNECTIONS      # MUTATED (baseline: MAX_SESSIONS, which exists)
...
# Keep timeouts short — Killercoda caps background-script runtime, and the
# mutated session-broker will never become Available (CreateContainerConfigError
# from the missing ConfigMap key). Don't block the whole setup on the broken one.
