...
# >>> M03 config enhancement: app-config ConfigMap, consumed by session-broker
#     both as env vars (envFrom) and as mounted files (/etc/app-config). This is
#     the healthy ConfigMap the baseline tours; breakfix-01 references a key it
#     doesn't have, breakfix-03 edits it after the pod is already running.
...
# >>> M03 config enhancement: consume app-config as env vars (envFrom) AND as
#     mounted files (/etc/app-config). Same object, both consumption modes.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
# >>> M03 config enhancement: portal-secrets, an Opaque Secret mounted into
#     portal-ui as files (/etc/portal). The baseline's volume-mounted-Secret
#     example; breakfix-02 mounts this same secret but never creates it.
kubectl create secret generic portal-secrets \
  ...
  -n admin-portal --dry-run=client -o yaml | kubectl apply -f - >/dev/null
# <<< M03 config enhancement ends
...
spec:
  ...
  template:
    ...
    spec:
      ...
      volumes:
        - name: portal-secrets
          secret: { secretName: portal-secrets }
---
apiVersion: v1
kind: Service
metadata:
