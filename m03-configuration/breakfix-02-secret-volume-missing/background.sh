...
# >>> breakfix-02 mutation: portal-ui mounts a Secret named portal-secrets as a
#     volume — but that Secret is NEVER created (the baseline's
#     `kubectl create secret generic portal-secrets ...` is intentionally absent
#     here). Volume setup is a precondition to container creation, so the pods
#     stall in ContainerCreating with a FailedMount event, no container ever run.
#     Models a manifest whose Secret was forgotten, renamed, or applied to the
#     wrong namespace.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
      ...
      containers:
        - name: nginx
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
          volumeMounts:
            - { name: portal-secrets, mountPath: /etc/portal, readOnly: true }
      volumes:
        - name: portal-secrets
          secret: { secretName: portal-secrets }   # MUTATED (baseline creates this Secret; here it is absent)
---
apiVersion: v1
kind: Service
metadata:
...
# mutated portal-ui will never become Available (stuck in ContainerCreating on a
# FailedMount for the absent Secret). Don't block the whole setup on the broken one.
