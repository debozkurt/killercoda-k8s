...
# >>> M14 addition: install the ingress-nginx controller (the north-south front door).
#     Kicked off here, before the fleet, so its image pull overlaps fleet creation;
#     we wait for it near the end. Pinned to a known-good release. It runs as a
#     NodePort Service in its own `ingress-nginx` namespace with IngressClass `nginx`.
...
# Drop the admission webhook: in a time-boxed lab its cert may not be ready when we
# apply the Ingress below, and it only validates syntax we already trust.
kubectl delete validatingwebhookconfiguration ingress-nginx-admission --ignore-not-found >/dev/null 2>&1
# <<< M14 addition ends
...
# >>> M14 addition: healthy policy + ingress state (this is the known-good tour).
# NetworkPolicy in `media`: a default-deny for ingress, then one allow that lets the
# app-services plane reach session-broker. Result — app-services in, everyone else out.
cat <<'EOF' | kubectl apply -f -
...
metadata:
  ...
  namespace: media
spec:
  podSelector: {}          # selects every pod in `media`
  policyTypes: [Ingress]   # …deny all ingress (no rules follow)
---
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
...
spec:
  ...
  ingress:
    - from:
        # namespaceSelector reaches across namespaces; every ns carries the
        # automatic kubernetes.io/metadata.name label, the reliable "namespace X" handle.
        - namespaceSelector: { matchLabels: { kubernetes.io/metadata.name: app-services } }
      ports:
        - { protocol: TCP, port: 80 }
EOF

# Ingress: route external HTTP for portal.polyphone.example to the portal-ui Service.
# Wait for the controller to be ready first so the object is claimed and gets an ADDRESS.
kubectl wait --namespace ingress-nginx \
  --for=condition=Available deployment/ingress-nginx-controller --timeout=180s >/dev/null 2>&1
cat <<'EOF' | kubectl apply -f -
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
      ...
      http:
        paths:
          - path: /
            ...
            backend:
              service:
                name: portal-ui       # a real Service in admin-portal…
                port: { number: 80 }  # …on a port it actually exposes
EOF
# <<< M14 addition ends
