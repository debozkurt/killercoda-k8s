...
# >>> breakfix-03 setup: install the ingress-nginx controller (needed for the Ingress
#     below to actually route). Kicked off before the fleet so the image pull overlaps.
...
# Keep timeouts short — Killercoda caps background-script runtime. The mutation breaks
# Ingress routing, not Pod health, so every fleet Pod still becomes Ready.
...
# Infra prereq: wait for the ingress controller before applying the Ingress, so it's
# claimed. This is the controller install (not the mutated workload) — the image pull
# can be slow on first boot, hence the longer ceiling.
kubectl wait --namespace ingress-nginx \
  --for=condition=Available deployment/ingress-nginx-controller --timeout=180s >/dev/null 2>&1

# >>> breakfix-03 mutation: the portal Ingress forwards to portal-ui on port 8080,
#     but the portal-ui Service only exposes port 80. The controller claims the Ingress
#     (class nginx) and matches the host — but it can find no endpoints for the backend
#     port 8080, so it has nothing to forward to and answers 503. The Service, its Pods,
#     and its endpoints are all healthy on 80; only the port the rule names is wrong.
#     Fix = point the backend port at 80 (what the Service actually exposes).
cat <<'EOF' | kubectl apply -f -
        ...
        paths:
          - path: /
            ...
            backend:
              service:
                name: portal-ui
                port: { number: 8080 }  # MUTATED (baseline: 80 — the only port portal-ui exposes)
EOF
