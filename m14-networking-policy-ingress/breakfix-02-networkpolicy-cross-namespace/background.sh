...
# breaks policy, not Pod health, so every Pod still becomes Ready.
...
# >>> breakfix-02 mutation: `media` has a default-deny for ingress plus an allow that
#     was MEANT to let sip-app (in the app-services namespace) reach session-broker.
#     But the allow's `from` peer is a bare podSelector with NO namespaceSelector — and
#     a bare podSelector is namespace-local: it means "pods labeled app=sip-app in the
#     policy's own namespace (media)", of which there are none. So the allow matches
#     nothing and the cross-namespace caller is denied. Pods/Service/DNS all healthy;
#     the request times out. Fix = add a namespaceSelector (AND) so the peer reaches
#     across the boundary to app-services.
cat <<'EOF' | kubectl apply -f -
...
spec:
  ...
  ingress:
    - from:
        # MUTATED (baseline: a namespaceSelector for app-services). A bare podSelector
        # never leaves this namespace, so this allow selects no real caller.
