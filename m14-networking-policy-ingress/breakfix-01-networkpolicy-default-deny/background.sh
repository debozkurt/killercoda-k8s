...
# >>> breakfix-01 mutation: `media` is locked down with a default-deny for ingress,
#     but NO companion allow was ever added. A NetworkPolicy selecting a pod flips it
#     to default-deny for that direction — so with only this policy, session-broker
#     (and every other pod in media) accepts ingress from NOBODY, including callers in
#     its own namespace. Pods stay Running/Ready, the Service keeps its endpoints, DNS
#     still resolves (this policy governs ingress only) — the connection just times out.
#     Baseline had this default-deny PLUS an allow; here the allow is missing.
#     Fix = add an allow (do NOT delete the deny).
cat <<'EOF' | kubectl apply -f -
...
spec:
  ...
  policyTypes: [Ingress]   # deny all ingress — and no allow policy accompanies it
EOF
