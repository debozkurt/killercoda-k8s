...
# session-cache is healthy here; the break is in call-coordinator's leader election.
# ===========================================================================
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: Service
...
# call-coordinator — the leader-elected singleton, with its election RBAC broken.
# >>> breakfix-03 mutation: the leader-election Role grants only list+watch on
#     leases — the get/create/update verbs the election client needs to acquire and
#     renew the Lease lock are missing. The client is forbidden from the lock object,
#     so it can never become leader: no Lease is ever held, and the active side of
#     call-coordinator never starts. The Pods themselves run (nginx), which is the
#     trap — the workload is up but idle, waiting on a leadership it cannot take.
#     Fix = restore get/create/update (and patch/delete) on leases to the Role.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ServiceAccount
...
rules:
  # MUTATED (baseline: get,list,watch,create,update,patch,delete) — only list+watch
  # remain, so the election client cannot get/create/update the Lease lock.
...
# No Lease is created: with the election client forbidden from the lock, leadership
# was never acquired, so `kubectl get lease call-coordinator -n call-routing` is empty.
