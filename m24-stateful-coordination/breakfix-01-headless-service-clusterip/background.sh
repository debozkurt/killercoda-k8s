...
# ===========================================================================
# M24 enhancement — stateful-coordination workloads (session-cache + call-coordinator)
# >>> breakfix-01 mutation: session-cache's governing Service is created WITHOUT
#     `clusterIP: None`, so it is an ordinary ClusterIP Service instead of a
#     headless one. Per-Pod stable DNS records (session-cache-0.session-cache...)
#     exist ONLY for a headless governing Service, so they vanish: peers can no
#     longer resolve a specific member by name. The name session-cache.media.svc
#     now returns one round-robin VIP — the wrong tool for addressing a named
#     member. The StatefulSet's Pods run fine; only per-Pod discovery is broken.
#     `clusterIP` is immutable, so the fix is delete + recreate the Service headless.
# ===========================================================================
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: Service
...
spec:
  # MUTATED (baseline: clusterIP: None) — the `clusterIP: None` line is removed, so
  # this Service gets a real VIP and stops publishing per-Pod stable DNS records.
...
# call-coordinator — leader-elected singleton (healthy in this scenario)
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ServiceAccount
...
# The Lease that records the current leader (call-coordinator is healthy here).
cat <<'EOF' | kubectl apply -f -
