...
# ===========================================================================
# >>> M24 enhancement: stateful-coordination workloads layered onto the fleet
# ---------------------------------------------------------------------------
#   session-cache    — a 3-replica StatefulSet (a replicated in-memory session
#                      cache) fronted by a HEADLESS Service for per-Pod DNS,
#                      using the default OrderedReady lifecycle and a per-Pod PVC
#                      (volumeClaimTemplates) as each member's persistent cache.
#   call-coordinator — a 2-replica active/standby singleton that elects one
#                      leader through a coordination.k8s.io Lease; ships its
#                      ServiceAccount + leader-election Role/RoleBinding and the
#                      Lease that records the current holder.
#   These are the subjects of the module's three break/fix scenarios. Appended
#   after the base-fleet wait so a capped wait on session-cache can't stall setup.
# ===========================================================================

# session-cache — headless Service + StatefulSet (media plane)
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: Service
metadata:
...
spec:
  clusterIP: None                 # headless — publishes per-Pod DNS, no VIP
...
metadata:
  ...
  labels: { app: session-cache, plane: media, tier: lab }
spec:
  serviceName: session-cache      # the governing headless Service (per-Pod DNS)
  replicas: 3
      ...
      containers:
        - name: app
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
          volumeMounts:
            - { name: data, mountPath: /var/cache/session }
  volumeClaimTemplates:
    - metadata: { name: data }
      spec:
        accessModes: [ReadWriteOnce]
...
# call-coordinator — leader-elected active/standby singleton (call-routing plane)
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ServiceAccount
metadata: { name: coordinator, namespace: call-routing }
---
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata: { name: leader-election, namespace: call-routing }
rules:
  # The leader-election client needs to read, create, and renew the Lease lock.
...
kind: Deployment
metadata:
  name: call-coordinator
  namespace: call-routing
  labels: { app: call-coordinator, plane: control, tier: lab }
spec:
  replicas: 2
  selector: { matchLabels: { app: call-coordinator } }
  template:
    metadata:
      labels: { app: call-coordinator, plane: control, tier: lab }
    spec:
...
# Bring the coordination workloads up (capped so a wedge can never stall setup).
kubectl rollout status statefulset/session-cache -n media --timeout=150s >/dev/null 2>&1
kubectl rollout status deployment/call-coordinator -n call-routing --timeout=90s >/dev/null 2>&1

# The Lease that records the current leader. In a live controller the election
# client creates and renews this; here the tour stages it so its shape is readable.
# Baseline: held by one replica, renewTime = now (a freshly-renewed, healthy lock).
cat <<'EOF' | kubectl apply -f -
apiVersion: coordination.k8s.io/v1
kind: Lease
metadata: { name: call-coordinator, namespace: call-routing }
spec:
  holderIdentity: pending-election
  leaseDurationSeconds: 15
EOF
# Stamp the holder + a fresh renewTime so the Lease reads as a healthy, freshly
# renewed lock. Applied via patch (JSON), not YAML, so the manifest stays literal.
HOLDER=$(kubectl get pods -n call-routing -l app=call-coordinator \
  -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)
NOW=$(date -u +%Y-%m-%dT%H:%M:%S.000000Z)
kubectl patch lease call-coordinator -n call-routing --type=merge \
  -p "{\"spec\":{\"holderIdentity\":\"${HOLDER:-call-coordinator-0}\",\"acquireTime\":\"${NOW}\",\"renewTime\":\"${NOW}\"}}" >/dev/null 2>&1
# <<< M24 enhancement ends
