...
# workload is layered AFTER this wait (below), so it never stalls the healthy-fleet
# readiness check.
...
# >>> breakfix-01 mutation: layer session-store, a 3-replica clustered session/registration
#     cache that needs stable per-member identity — a textbook StatefulSet. The StatefulSet
#     names `serviceName: session-store` as its governing Service, but that headless Service
#     is NOT created here (the Service block a healthy StatefulSet ships alongside is omitted).
#     A StatefulSet does NOT create its governing Service — you must — so the Pods start and
#     run fine (identity/storage are intact) while the per-Pod DNS records are never published:
#     `session-store-0.session-store.app-services.svc.cluster.local` returns NXDOMAIN and the
#     members can't discover each other. Layered after the fleet wait so it doesn't gate it.
#     Fix = create the headless Service (clusterIP: None, selector app=session-store) that
#     `serviceName` points at, and the per-Pod DNS records appear.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: StatefulSet
metadata:
...
spec:
  # serviceName references the headless Service `session-store` — which is MUTATED
  # (baseline: a headless Service `session-store`, clusterIP: None, is created alongside) —
  # omitted here, so per-Pod DNS is never served.
  ...
  volumeClaimTemplates:
    - metadata: { name: state }
      spec:
        ...
        resources: { requests: { storage: 100Mi } }
EOF
# Give the 3 ordinals time to come up (they're healthy — only DNS is broken).
kubectl rollout status statefulset/session-store -n app-services --timeout=90s >/dev/null 2>&1
