...
# workload is layered AFTER this wait (below); it comes up healthy on the worker, so
# this is just for consistency with the other scenarios.
...
# >>> breakfix-03 mutation: layer rtp-probe, a per-node RTP media-quality probe meant to run
#     on EVERY node (a textbook DaemonSet, like sbc-edge). The break is that its Pod template
#     is MISSING the toleration for the control-plane taint that sbc-edge carries. A DaemonSet
#     only counts a node in desiredNumberScheduled if the Pod matches the node's selectors AND
#     tolerates its taints — so the tainted control-plane node is NOT eligible, no Pod is ever
#     created for it, and the DaemonSet silently covers only the worker: `get ds` shows
#     DESIRED 1 on a 2-node cluster, with no error and no Pending Pod. Contrast: sbc-edge,
#     which DOES tolerate the control-plane taint, shows DESIRED 2. Layered after the fleet wait.
#     Fix = add the control-plane toleration so the node becomes eligible and a Pod lands there.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: DaemonSet
metadata:
  name: rtp-probe
  namespace: edge
  labels: { app: rtp-probe, plane: edge, tier: lab }
spec:
  selector: { matchLabels: { app: rtp-probe } }
  template:
    metadata:
      labels: { app: rtp-probe, plane: edge, tier: lab }
    spec:
      # MUTATED (baseline: a toleration for node-role.kubernetes.io/control-plane:NoSchedule,
      #   matching what sbc-edge carries) — omitted, so the tainted control-plane node is not
      #   eligible and this DaemonSet covers only the worker (DESIRED 1 on a 2-node cluster).
      containers:
        - name: app
          image: nginx:1.25
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
EOF
kubectl rollout status daemonset/rtp-probe -n edge --timeout=60s >/dev/null 2>&1
