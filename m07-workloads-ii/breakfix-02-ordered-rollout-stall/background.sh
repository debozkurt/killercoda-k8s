...
# workload is layered AFTER this wait (below); it deliberately never goes fully
# Ready, so it must not gate the healthy-fleet readiness check.
...
# >>> breakfix-02 mutation: layer session-store, a 3-replica clustered session cache
#     (a textbook StatefulSet), WITH its headless Service intact — identity is fine here.
#     The break is a readiness probe pointed at port 8080, but the container serves on 80,
#     so the probe never succeeds and the Pod is Running-but-never-Ready. Under the default
#     podManagementPolicy: OrderedReady, the controller will not create ordinal N+1 until
#     ordinal N is Running AND Ready — so session-store-0 comes up 0/1 Ready and session-store-1
#     and -2 are NEVER created. Symptom: `get statefulset` shows READY 0/3 with only one Pod.
#     Layered after the fleet wait so its stall doesn't gate the healthy fleet.
#     Fix = point the readiness probe at the port the container actually serves (80); Pod-0
#     goes Ready and the controller unblocks and creates -1 then -2.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: Service
metadata:
    ...
    spec:
      containers:
        - name: app
          ...
          readinessProbe:
            httpGet: { path: /, port: 8080 }   # MUTATED (baseline: port: 80) — nginx serves on 80, so the probe on 8080 is refused and the Pod never goes Ready
            periodSeconds: 5
          resources:
            requests: { cpu: 25m, memory: 32Mi }
            limits:   { cpu: 100m, memory: 64Mi }
  ...
  volumeClaimTemplates:
    - metadata: { name: state }
      spec:
        ...
        resources: { requests: { storage: 100Mi } }
EOF
# Wait (bounded) for ordinal 0 to exist so the stalled state is visible on first look.
for i in $(seq 1 15); do
  kubectl get pod session-store-0 -n app-services >/dev/null 2>&1 && break
  sleep 2
done
