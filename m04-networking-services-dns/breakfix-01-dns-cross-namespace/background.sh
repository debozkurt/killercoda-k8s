...
# >>> breakfix-01 mutation: account-provisioner is told to reach the session
#     broker at the BARE name "session-broker". But session-broker lives in the
#     `media` namespace and this caller is in `provisioning` — a short name only
#     resolves within the caller's own namespace, so it expands to
#     session-broker.provisioning.svc.cluster.local and returns NXDOMAIN. The pod
#     itself stays Running (nginx never dials the endpoint); the failure is in
#     name resolution, reproduced from this namespace. Fix = qualify the name.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
        ...
        - name: app
          ...
          env:
            - name: BROKER_ENDPOINT
              value: http://session-broker/   # MUTATED (baseline: http://session-broker.media.svc.cluster.local/)
          ports: [{ containerPort: 80 }]
          resources:
            requests: { cpu: 25m, memory: 32Mi }
            limits:   { cpu: 100m, memory: 64Mi }
...
# breaks Service/DNS routing, not Pod health, so every Pod still becomes ready.
