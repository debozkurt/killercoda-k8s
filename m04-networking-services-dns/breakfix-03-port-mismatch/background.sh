...
# >>> breakfix-03 mutation: the portal-ui Service forwards to targetPort 8080,
#     but the nginx container listens on 80. The selector is correct and the
#     EndpointSlice IS populated (Pods are Ready) — but traffic is delivered to a
#     port with no listener, so the Pod's kernel sends a RST and the client sees
#     "connection refused". Distinct from breakfix-02: endpoints present, wrong
#     port. containerPort: 80 is just documentation; it opens nothing. Fix =
#     point targetPort at the real listener (80).
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  ports: [{ port: 80, targetPort: 8080 }]   # MUTATED (baseline: targetPort: 80 — nginx listens on 80, nothing on 8080)
  type: ClusterIP
EOF
