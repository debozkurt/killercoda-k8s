...
The admin portal is down — `portal-ui` in `admin-portal` refuses every connection. Your first instinct after the last scenario is right: check the endpoints. But this time `kubectl get endpoints portal-ui` is **populated** — the Pods are there, Ready, and in the EndpointSlice. The traffic is reaching a Pod and getting bounced.
