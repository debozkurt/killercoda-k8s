...
# >>> breakfix-02 mutation: the route-engine Service selector is set to a label
#     no Pod carries (app: route-enginev2), while the Pods stay labeled
...
metadata:
  ...
  labels: { app: route-engine, plane: control, tier: lab }
spec:
  selector: { app: route-enginev2 }   # MUTATED (baseline: app: route-engine — no Pod has this label)
  ports: [{ port: 80, targetPort: 80 }]
  type: ClusterIP
EOF
