...
You found a black hole: a Service with a ClusterIP, healthy Pods, no errors anywhere — and an empty EndpointSlice routing traffic to nothing. The `get svc` headline was green; `get endpoints` told the truth. The cause was a selector that matched no Pod's labels, and the fix touched only the Service — the Pods were never the problem.
