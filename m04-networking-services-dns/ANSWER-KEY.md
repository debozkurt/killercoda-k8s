...
- **Step 2 (the Pod's own view):** from a `busybox` probe, `ip addr show eth0` reports the same address `get -o wide` did, plus a default route to the node — one interface, one routing table, nothing of the node's. `kubectl exec` into each of `call-recorder`'s two containers returns the **same** `inet` line, and `uploader` reaches `recorder` at `localhost:8080` (`recorder-ok`): one network namespace per Pod, shared by its containers, which is why two containers can never both bind a port. `ls /etc/cni/net.d/` names the plugin that installed the interface, IP, and route.
