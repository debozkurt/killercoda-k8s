...
> Environment: Killercoda `kubernetes-kubeadm-2nodes` with the Polyphone baseline, plus one layered workload — `call-recorder` in `media`, the fleet's only multi-container Pod. The break/fix scenarios mutate existing fleet Services (and one DNS env carrier on `account-provisioner`). Traffic is driven from a throwaway `busybox` client.
...
- **Step 2 (the Pod's own view):** from a `busybox` probe, `ip addr show eth0` reports the same address `get -o wide` did — one interface, one routing table, nothing of the node's. Four readable details: `eth0@ifNN` names the **veth peer index** on the node side; the `/32` mask means no on-link subnet, so every packet goes to the gateway (plugins that use a bridge instead don't look like this); the `default via` address sits in the Pod range, so it is the CNI's gateway on this node rather than the node's NIC; and `mtu 1400` is encapsulation headroom, the usual suspect when small requests work and large ones hang. `kubectl exec` into each of `call-recorder`'s two containers returns the **same** `inet` line, and `uploader` reaches `recorder` at `localhost:8080` (`recorder-ok`): one network namespace per Pod, shared by its containers, which is why two containers can never both bind a port. `ls /etc/cni/net.d/` names the plugin that installed the interface, IP, and route.
- **Step 3 (Service identity & ClusterIP):** `kubectl get svc session-broker -n media` shows a ClusterIP out of the Service range and PORT(S) 80/TCP. A client `wget`'ing `http://session-broker/` from a Pod in `media` gets the same HTML — the Service answers, fronting a Pod whose own IP the client never sees.
- **Step 4 (selector → EndpointSlice):** `kubectl get endpointslice -n media -l kubernetes.io/service-name=session-broker` lists the Pod address(es) on port 80; `describe` on the slice adds each endpoint's `Ready: true`. `kubectl describe svc session-broker -n media` shows `Selector:` and `Endpoints:` together — the query and its answer. Teaching point: `get svc` proves existence, the endpoints prove reachability.
- **Step 5 (who owns the EndpointSlice):** scaling to two replicas adds an address with no edit to the Service. Deleting the slice gets it rebuilt under a new name with the same addresses — the controller owns it, not you. Give it a few seconds: reading straight after the delete shows the gap, not the result. Scaling back leaves one address.
- **Step 6 (`port` vs `targetPort`):** `kubectl describe svc session-broker -n media` reads `Port: 80/TCP`, `TargetPort: 80/TCP`, and the endpoint on :80. `kubectl describe pod` shows the container's `Port: 80/TCP` — the `containerPort` declaration, which is documentation only; the listener is what matters. `kubectl port-forward svc/session-broker 8080:80 -n media` then a local `curl localhost:8080` also answers, using neither the ClusterIP nor cluster DNS.
