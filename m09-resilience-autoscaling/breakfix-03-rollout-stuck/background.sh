...
# Keep timeouts short — Killercoda caps background-script runtime. The scenario's
# workload is applied AFTER this wait (below); its old replicas keep serving during the
# stuck rollout, so it never stalls the healthy-fleet readiness check.
...
# >>> breakfix-03 mutation: layer an admin workload, portal-web, healthy at nginx:1.25
#     (revision 1), then push a rolling update to a NON-EXISTENT image tag
#     (nginx:1.25-doesnotexist, revision 2). This scenario is explicitly ABOUT a
#     post-start change — a rollout — so, unlike the deploy-broken-from-start rule, it
#     intentionally creates good then updates to broken, leaving a real revision history
#     for `kubectl rollout undo` to return to. The new ReplicaSet's Pods can't pull the
#     image (ImagePullBackOff); with the default rolling-update strategy (maxUnavailable
#     rounds down to 0 for 2 replicas) the two old Pods keep serving, so the app stays
#     Available while the rollout hangs and, after progressDeadlineSeconds (60), reports
#     Progressing=False / ProgressDeadlineExceeded. Kubernetes does NOT auto-roll-back.
#     Hardened: apply, wait for rev 1, set the bad image, read back, fail loudly if the
#     update didn't register.
#     Fix = kubectl rollout undo (return to the last good revision).
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  name: portal-web
  namespace: admin-portal
  labels: { app: portal-web, plane: admin, tier: lab }
  annotations:
    kubernetes.io/change-cause: "rev1: nginx:1.25 (known-good baseline)"
spec:
  replicas: 2
  progressDeadlineSeconds: 60
  selector: { matchLabels: { app: portal-web } }
  template:
    metadata:
      labels: { app: portal-web, plane: admin, tier: lab }
    spec:
      containers:
        - name: app
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
EOF
# Wait for revision 1 to roll out fully before pushing the bad update, so a real
# "last good revision" exists for rollout undo to land on.
kubectl rollout status deployment/portal-web -n admin-portal --timeout=90s >/dev/null 2>&1

# MUTATED (baseline: image stays nginx:1.25) — push revision 2 to a non-existent tag.
kubectl set image deployment/portal-web app=nginx:1.25-doesnotexist -n admin-portal >/dev/null 2>&1
kubectl annotate deployment/portal-web -n admin-portal \
  kubernetes.io/change-cause="rev2: nginx:1.25-doesnotexist (bad tag - stuck rollout)" --overwrite >/dev/null 2>&1
...
if [ "$IMG" != "nginx:1.25-doesnotexist" ]; then
  echo "FATAL: portal-web bad rollout did not register (image is '$IMG')." >&2
  echo "       The stuck-rollout scenario depends on revision 2 pointing at a bad tag." >&2
  exit 1
fi
