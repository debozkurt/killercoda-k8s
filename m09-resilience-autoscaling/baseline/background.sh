...
# ---------------------------------------------------------------------------
# M09 baseline additions — the healthy resilience objects this tour reads.
# Not broken; the break/fix scenarios each break one of these in isolation.
# ---------------------------------------------------------------------------

# A healthy HorizontalPodAutoscaler on sip-router (2 replicas, CPU request 25m each).
# The target HAS a CPU request, so HPA can compute utilization and TARGETS shows a real
# percentage (idle → a few %/50%) rather than <unknown>. metrics-server is installed
# above; it takes ~15-30s after boot to report the first sample.
cat <<'EOF' | kubectl apply -f -
...
# A healthy PodDisruptionBudget on route-engine (2 replicas). minAvailable: 1 leaves
# headroom for one voluntary disruption at a time: ALLOWED DISRUPTIONS = 2 - 1 = 1, so
# a node drain can evict one replica while the other keeps serving.
cat <<'EOF' | kubectl apply -f -
...
spec:
  minAvailable: 1
  selector:
    matchLabels: { app: route-engine }
EOF

# Give metrics-server a moment so the HPA has a sample to report during the tour.
kubectl wait --for=condition=Available deployment/metrics-server -n kube-system --timeout=60s >/dev/null 2>&1
