...
# >>> breakfix-01 mutation: layer a signaling workload, sip-registrar, at 2 replicas,
#     guarded by a PodDisruptionBudget whose minAvailable EQUALS the replica count.
#     The Deployment itself is healthy (2/2 Running) — the PDB is the broken object.
#     ALLOWED DISRUPTIONS = currentHealthy(2) - minAvailable(2) = 0, so the eviction
#     API refuses every voluntary eviction: a `kubectl drain` of the worker for a
#     kernel patch would block indefinitely on this budget. Baked broken at apply time
#     (no post-apply patch). Applied after the fleet wait; sip-registrar comes up clean.
#     Fix = lower minAvailable below the replica count (e.g. 1) so the budget permits
...
metadata:
  name: sip-registrar
  namespace: signaling
  labels: { app: sip-registrar, plane: signaling, tier: lab }
spec:
  replicas: 2
  selector: { matchLabels: { app: sip-registrar } }
  template:
    metadata:
      labels: { app: sip-registrar, plane: signaling, tier: lab }
    spec:
...
metadata:
  ...
  labels: { app: sip-registrar, plane: signaling, tier: lab }
spec:
  # MUTATED (baseline: minAvailable: 1) — minAvailable equal to the replica count
  # leaves zero headroom: ALLOWED DISRUPTIONS = healthy(2) - minAvailable(2) = 0, so
  # the eviction API refuses to release any replica and a node drain is blocked.
  minAvailable: 2
  selector:
    matchLabels: { app: sip-registrar }
EOF
