...
The reflex is to blame the metrics pipeline, but metrics-server is installed and healthy here — `kubectl top pods -n media` returns live CPU for the Pod. The problem is on the *target* side: the HPA is being asked to compute a percentage it has no way to compute.
