...
# objects are applied AFTER this wait (below); the layered workload comes up clean,
# so it never stalls the healthy-fleet readiness check.
...
# >>> breakfix-02 mutation: layer a media workload, transcode-scaler, fronted by a
#     HorizontalPodAutoscaler that targets 50% CPU utilization — but the Deployment's
#     container declares NO CPU request. HPA computes CPU *utilization* as usage ÷
#     request; with no request there is no denominator, so the autoscaler can't read a
#     metric: `kubectl get hpa` shows TARGETS <unknown>/50% and it never scales
#     (ScalingActive False, FailedGetResourceMetric). metrics-server is installed, so
#     the metrics pipeline is otherwise healthy — the missing request is the whole bug.
#     Baked broken at apply time; the Deployment runs fine (1/1), only autoscaling is dead.
#     Fix = add a CPU request to the container so HPA has a denominator for the percentage.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  name: transcode-scaler
  namespace: media
  labels: { app: transcode-scaler, plane: media, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: transcode-scaler } }
  template:
    metadata:
      labels: { app: transcode-scaler, plane: media, tier: lab }
    spec:
      containers:
        - name: app
          image: nginx:1.25
          ports: [{ containerPort: 80 }]
          # MUTATED (baseline: requests: { cpu: 100m, memory: 64Mi }) — the CPU request
          # was dropped. HPA has no denominator for CPU utilization and reads <unknown>.
          # Only a memory limit remains, which the CPU metric ignores.
          resources:
            limits: { memory: 128Mi }
---
