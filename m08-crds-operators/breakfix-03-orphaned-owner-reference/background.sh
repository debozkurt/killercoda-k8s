...
# ===========================================================================
# M08 platform additions — the CRD + operator + tenant CRs this module teaches.
# A CustomResourceDefinition (MediaTenant) extends the API with a new noun; a
# small controller (tenant-operator) reconciles each MediaTenant into a child
# media Deployment, stamps an ownerReference back to the CR, and reports progress
# in .status. Nothing here is broken — this is the healthy reference the tour reads.
# ===========================================================================

kubectl create namespace platform --dry-run=client -o yaml | kubectl apply -f - >/dev/null

# --- The CustomResourceDefinition: teach the API server a new type ----------
cat <<'EOF' | kubectl apply -f -
...
# --- Operator RBAC: the ServiceAccount the controller authenticates as, and
#     the ClusterRole granting exactly the verbs its reconcile loop needs:
#     read MediaTenants + write their /status, and manage child Deployments.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ServiceAccount
metadata:
...
# --- The operator: a control loop that reconciles every MediaTenant into a
#     child media Deployment (owner-referenced back to the CR) and writes .status.
#     Deliberately a small kubectl reconcile loop so the pattern is legible, not a
#     compiled controller. `kubectl` in a Pod uses the mounted ServiceAccount.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
# --- The tenants: two MediaTenant CRs the operator reconciles into capacity.
cat <<'EOF' | kubectl apply -f -
...
# Give the operator a moment to reconcile both tenants into child Deployments.
for i in $(seq 1 40); do
  kubectl get deployment orion-media -n media >/dev/null 2>&1 && \
  kubectl get deployment lyra-media  -n media >/dev/null 2>&1 && break
  sleep 3
done
kubectl rollout status deployment/orion-media -n media --timeout=90s >/dev/null 2>&1
kubectl rollout status deployment/lyra-media  -n media --timeout=90s >/dev/null 2>&1

# >>> breakfix-03 mutation: tenant `vega` was offboarded weeks ago — its MediaTenant CR
#     was deleted. Its child media Deployment, vega-media, should have been cascade-deleted
#     with it, but it is still Running and holding capacity. The reason: vega-media was
#     created by an older operator (before owner-reference stamping) and carries NO
#     ownerReferences, so the garbage collector had no owner link to follow when the CR was
#     deleted — the child was orphaned, not collected. We reproduce that exact state by
#     applying vega-media with no ownerReferences and no `vega` MediaTenant. The current
#     operator only manages Deployments for MediaTenants that EXIST, so it never touches
#     this orphan. Contrast: orion-media and lyra-media DO carry an ownerReference to their
#     MediaTenant (the operator stamped it). Fix = delete the orphan to reclaim its capacity.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  name: vega-media
  namespace: media
  # MUTATED (baseline: a healthy child carries ownerReferences → its MediaTenant, as
  #   orion-media does) — this orphan has NO ownerReferences and no parent CR, so cascading
  #   GC never removed it when tenant vega was offboarded.
  labels: { app: vega-media, managed-by: tenant-operator, plane: media, tier: lab }
spec:
  replicas: 2
  selector: { matchLabels: { app: vega-media } }
  template:
    metadata:
      labels: { app: vega-media, plane: media, tier: lab }
    spec:
      containers:
        - name: nginx
          image: nginx:1.25
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
EOF
kubectl rollout status deployment/vega-media -n media --timeout=60s >/dev/null 2>&1
