...
# >>> breakfix-01 mutation: layer a new analytics workload, stream-analyzer, whose
#     memory REQUEST was fat-fingered from 256Mi to 256Gi (a Mi->Gi unit slip). The
#     scheduler fits Pods by their *requests* against each node's Allocatable; no node
#     has 256Gi, so the Pod is rejected by every node and stays Pending with a
...
metadata:
  ...
  namespace: analytics
  labels: { app: stream-analyzer, plane: control, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: stream-analyzer } }
  template:
    metadata:
      labels: { app: stream-analyzer, plane: control, tier: lab }
    spec:
