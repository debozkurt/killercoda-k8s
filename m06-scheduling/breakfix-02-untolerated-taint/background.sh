...
# workload (and node taint) are applied AFTER this wait (below), so they never stall
...
# >>> breakfix-02 mutation: taint the worker node to represent a dedicated telephony
#     node pool, then layer a new edge workload, pstn-probe, that is MISSING the
#     matching toleration. A NoSchedule taint repels any Pod that doesn't tolerate it;
#     the worker now carries `dedicated=telephony:NoSchedule` and the control-plane
#     still carries its built-in `node-role.kubernetes.io/control-plane:NoSchedule`, so
#     pstn-probe (tolerating neither) fits on no node and stays Pending with a
#     FailedScheduling event ("untolerated taint"). NoSchedule does NOT evict the
#     already-running fleet — only new scheduling is blocked — which is why the rest of
#     the cluster is unaffected. Taint + workload applied after the fleet is up.
#     Fix = add a toleration for dedicated=telephony (matching what sbc-edge does for
#     the control-plane taint), so the Pod can land on the worker.
WORKER=$(kubectl get nodes -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.metadata.labels.node-role\.kubernetes\.io/control-plane}{"\n"}{end}' | awk '$2=="" {print $1; exit}')
...
metadata:
  name: pstn-probe
  namespace: edge
  labels: { app: pstn-probe, plane: edge, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: pstn-probe } }
  template:
    metadata:
      labels: { app: pstn-probe, plane: edge, tier: lab }
    spec:
      # MUTATED (baseline: a toleration for dedicated=telephony:NoSchedule) — omitted,
      # so this Pod cannot schedule onto the tainted worker (nor the tainted control-plane).
      containers:
        - name: app
          image: nginx:1.25
          ports: [{ containerPort: 5060, name: sip }]
