...
# workload is applied AFTER this wait (below), so its CrashLoop Pod never stalls
...
# >>> breakfix-04 mutation: layer a new media workload, media-buffer, that pre-allocates
#     a ~60Mi in-memory jitter buffer (dd into a memory-backed emptyDir, which is charged
#     to the container's memory cgroup) and then holds it. Its memory LIMIT was set to
#     48Mi (baseline 128Mi). Requests fit the scheduler, so the Pod SCHEDULES fine — this
#     is not a Pending failure. At runtime it writes past its 48Mi limit, the kernel
#     OOM-kills the process (exit code 137), and the container restarts into
#     CrashLoopBackOff. QoS is Burstable (requests < limits). This is the runtime
#     counterpart to breakfix-01: requests are what you fit; limits are what kills you.
#     Fix = raise the memory limit above the working-set (128Mi), so the buffer fits.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  name: media-buffer
  namespace: media
  labels: { app: media-buffer, plane: media, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: media-buffer } }
  template:
    metadata:
      labels: { app: media-buffer, plane: media, tier: lab }
    spec:
      containers:
        - name: app
          image: busybox:1.36
          # Pre-allocate a ~60Mi in-memory ring buffer, then hold it resident.
          command: ["sh", "-c", "dd if=/dev/zero of=/buf/ring bs=1M count=60 && sleep 3600"]
          resources:
            requests: { cpu: 25m, memory: 32Mi }
            limits:   { cpu: 100m, memory: 48Mi }   # MUTATED (baseline: 128Mi) — 60Mi buffer > 48Mi limit -> OOMKilled, CrashLoopBackOff
          volumeMounts:
