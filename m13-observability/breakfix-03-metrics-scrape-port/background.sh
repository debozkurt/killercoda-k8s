...
# Keep timeouts short — Killercoda caps background-script runtime. call-metrics
# is applied AFTER this healthy-fleet wait; it comes up Running (the mutation is
# a scrape-port mismatch, not a broken Pod), so it doesn't stall anything.
...
# >>> breakfix-03 mutation: layer call-metrics in analytics — an nginx serving a
#     Prometheus /metrics endpoint on container port 80. The Pod's scrape
#     annotation prometheus.io/port says "9090", a port nothing listens on, so a
#     Prometheus scrapes 9090, gets connection-refused, and marks the target
#     DOWN — every graph for this workload goes flat. The app is Running 1/1 and
#     `kubectl top` (the OTHER pipeline) works fine; only the scrape is broken.
#     Fix = point prometheus.io/port at the port /metrics is actually served on (80).
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ConfigMap
metadata:
  name: call-metrics-exposition
  namespace: analytics
data:
  metrics: |
    # HELP sip_calls_active Currently active SIP calls handled by the fleet.
    # TYPE sip_calls_active gauge
    sip_calls_active 42
    # HELP sip_calls_total Total SIP calls processed since process start.
    # TYPE sip_calls_total counter
    sip_calls_total 18734
    # HELP sip_call_setup_seconds SIP call setup latency in seconds.
    # TYPE sip_call_setup_seconds histogram
    sip_call_setup_seconds_bucket{le="0.1"} 12000
    sip_call_setup_seconds_bucket{le="0.5"} 18000
    sip_call_setup_seconds_bucket{le="+Inf"} 18734
    sip_call_setup_seconds_sum 5123.4
    sip_call_setup_seconds_count 18734
---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: call-metrics
  namespace: analytics
  labels: { app: call-metrics, plane: control, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: call-metrics } }
  template:
    metadata:
      labels: { app: call-metrics, plane: control, tier: lab }
      annotations:
        prometheus.io/scrape: "true"
        prometheus.io/port: "9090"     # MUTATED (baseline: "80") — nothing listens on 9090; the scrape is refused, the target is DOWN
        prometheus.io/path: "/metrics"
    spec:
      containers:
        - name: app
          image: nginx:1.25
          ports: [{ containerPort: 80, name: metrics }]
          volumeMounts:
            - { name: exposition, mountPath: /usr/share/nginx/html/metrics, subPath: metrics }
          resources:
            requests: { cpu: 25m, memory: 32Mi }
            limits:   { cpu: 100m, memory: 64Mi }
      volumes:
        - name: exposition
          configMap: { name: call-metrics-exposition }
---
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  ports: [{ port: 80, targetPort: metrics, name: metrics }]
EOF
