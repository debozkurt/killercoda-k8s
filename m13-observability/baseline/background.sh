...
# >>> m13 baseline enhancement: layer a healthy application-metrics target,
#     call-metrics, in the analytics namespace. It's an nginx serving a static
#     Prometheus exposition-format document at /metrics (fed by a ConfigMap), on
#     container port 80 (named `metrics`). Its Pod template carries the scrape
#     annotations a Prometheus would discover it by — prometheus.io/scrape,
#     prometheus.io/port: "80", prometheus.io/path: "/metrics" — all pointing at
#     the port it actually serves on. This is what a healthy scrape target looks
#     like; break/fix 03 ships the same workload with a mismatched scrape port.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ConfigMap
metadata:
  ...
  template:
    metadata:
      ...
      annotations:
        ...
        prometheus.io/port: "80"       # matches the container port that serves /metrics
