...
# workload is applied AFTER this healthy-fleet wait (below), so it never stalls
# the readiness check.
...
# >>> breakfix-01 mutation: layer session-logger in app-services. It emits a
#     per-session log line every few seconds, but writes those lines to a FILE
#     inside the container (/var/log/app/session.log) instead of to stdout. The
#     kubelet only captures stdout/stderr, so `kubectl logs` shows only the
#     one-line startup banner and none of the session activity — the workload is
#     Running 1/1 and logging, but invisible. The log file sits on an emptyDir
#     shared with the (absent) sidecar slot, so either fix works: redirect the
#     app to stdout, or add a streaming sidecar that tails the file to its stdout.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  name: session-logger
  namespace: app-services
  labels: { app: session-logger, plane: app, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: session-logger } }
  template:
    metadata:
      labels: { app: session-logger, plane: app, tier: lab }
    spec:
      containers:
        - name: app
          ...
          args:
            - |
              echo "[session-logger] starting; writing session events to /var/log/app/session.log"
              mkdir -p /var/log/app
              i=0
              while true; do
                i=$((i+1))
                echo "$(date -u +%FT%TZ) session sess-$i established peer=10.10.0.$((i%254))" >> /var/log/app/session.log  # MUTATED (baseline: echoed to stdout, no '>> file' redirect)
                sleep 3
              done
          volumeMounts:
