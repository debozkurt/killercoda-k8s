...
spec:
  ...
  duration: 2160h
  # >>> breakfix-02 mutation: the server cert's SANs omit the name clients connect by.
  #     config-client calls https://config-api.media.svc.cluster.local, but this cert is
  #     only valid for config-api-legacy.* — so the cert issues Ready and the Secret exists,
  #     yet the TLS handshake fails hostname verification ("no alternative certificate
  #     subject name matches target host name 'config-api.media.svc.cluster.local'"). The
  #     fix is to put the real dnsNames back, then roll config-api so nginx reloads the cert.
  dnsNames:
    - config-api-legacy.media.svc.cluster.local   # MUTATED (baseline: config-api.media.svc.cluster.local + config-api.media.svc + config-api)
...
spec:
  ...
  issuerRef: { name: polyphone-ca, kind: ClusterIssuer, group: cert-manager.io }
EOF
kubectl wait --for=condition=Ready certificate/config-api-tls    -n media        --timeout=90s >/dev/null 2>&1
...
kubectl rollout status deployment/config-api    -n media        --timeout=90s >/dev/null 2>&1
