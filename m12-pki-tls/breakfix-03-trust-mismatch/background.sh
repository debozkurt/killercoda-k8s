  ...
  template:
    ...
    spec:
      ...
      volumes:
        ...
        - name: trust
          # >>> breakfix-03 mutation: mount the WRONG trust anchor. config-client verifies the
          #     server against whatever CA lives in this bundle; legacy-ca-bundle is an
          #     unrelated CA, so the server's cert (signed by the internal CA) chains to a root
          #     the client doesn't trust — curl fails with "unable to get local issuer
          #     certificate". The fix is to mount the correct bundle, internal-ca-bundle.
          secret: { secretName: legacy-ca-bundle }   # MUTATED (baseline: internal-ca-bundle)
