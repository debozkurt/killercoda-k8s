...
# >>> breakfix-01 mutation: layer an endpoint-discovery reader, endpoint-watcher, in
#     the media namespace. Its ServiceAccount (endpoint-watcher) is bound — via a
#     RoleBinding — to a Role that grants get + watch on endpoints but NOT list. The
#     reader does a `list` (a GET on the endpoints *collection*, which requires the
#     `list` verb), so the API server denies it with 403 Forbidden ("cannot list
#     resource endpoints ... in the namespace media"); the container exits non-zero
#     and CrashLoopBackOffs. The SA, the binding, and the app are all fine — the Role
#     is one verb too narrow. Fix = add `list` to the Role's verbs.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ServiceAccount
metadata:
...
rules:
  - apiGroups: [""]
    resources: ["endpoints"]
    verbs: ["get", "watch"]   # MUTATED (baseline: ["get","list","watch"]) — missing `list`, so the reader's collection GET is denied
---
apiVersion: rbac.authorization.k8s.io/v1
kind: RoleBinding
metadata:
  name: endpoint-watcher-binding
  namespace: media
roleRef:
...
metadata:
  name: endpoint-watcher
  namespace: media
  labels: { app: endpoint-watcher, plane: media, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: endpoint-watcher } }
  template:
    metadata:
      labels: { app: endpoint-watcher, plane: media, tier: lab }
    spec:
      serviceAccountName: endpoint-watcher
      containers:
        - name: app
