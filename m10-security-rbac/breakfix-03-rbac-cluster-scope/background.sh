...
# >>> breakfix-03 mutation: layer a node-inventory reader, node-inspector, in the
#     analytics namespace. It reads `nodes`, which are CLUSTER-scoped. The grant is
#     written with a namespaced Role + RoleBinding — which RBAC accepts as YAML, but a
#     namespaced binding can never reach a cluster-scoped resource, so the grant does
#     nothing. The reader lists nodes and is denied with 403 Forbidden ending
#     "... at the cluster scope" (not "in the namespace"). The container exits non-zero
#     and CrashLoopBackOffs. Fix = grant it with a ClusterRole + ClusterRoleBinding.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ServiceAccount
metadata:
...
# MUTATED (baseline: kind ClusterRole) — a namespaced Role can't grant the cluster-scoped `nodes`
kind: Role
metadata:
  name: node-reader
  namespace: analytics
...
---
apiVersion: rbac.authorization.k8s.io/v1
# MUTATED (baseline: kind ClusterRoleBinding referencing a ClusterRole) — a namespaced
# RoleBinding grants only within analytics and can't reach cluster-scoped nodes
kind: RoleBinding
metadata:
  name: node-inspector-binding
  namespace: analytics
roleRef:
...
metadata:
  name: node-inspector
  namespace: analytics
  labels: { app: node-inspector, plane: control, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: node-inspector } }
  template:
    metadata:
      labels: { app: node-inspector, plane: control, tier: lab }
    spec:
      serviceAccountName: node-inspector
      containers:
        - name: app
          ...
          args:
            - |
              ...
              while true; do
                ...
                echo "GET /api/v1/nodes -> HTTP $code"
                cat /tmp/body; echo
                if [ "$code" != "200" ]; then
                  echo "node inventory FAILED (HTTP $code) — exiting"
                  exit 1
                fi
                sleep 30
              done
