...
# Keep timeouts short — Killercoda caps background-script runtime. The hardened
# namespace and its workload are applied AFTER this wait (below), so the rejected
# Deployment never stalls the healthy-fleet readiness check.
...
# >>> breakfix-04 mutation: the security team stood up a hardened namespace, payments,
#     that ENFORCES the restricted Pod Security Standard, and rolled out payments-api
#     into it. The Deployment sets NO securityContext, so every Pod it tries to create
#     is rejected at admission ("violates PodSecurity restricted:latest": no
#     runAsNonRoot, allowPrivilegeEscalation != false, unrestricted capabilities, no
#     seccompProfile). The Deployment object is admitted (controllers aren't Pods), but
#     its ReplicaSet creates zero Pods and reports the violation as a FailedCreate event.
#     Deployment sits 0/1 with no Pods at all — not even Pending. Fix = add the
#     restricted-compliant securityContext to the Pod template.
kubectl create namespace payments --dry-run=client -o yaml | kubectl apply -f - >/dev/null
kubectl label namespace payments \
  pod-security.kubernetes.io/enforce=restricted \
  pod-security.kubernetes.io/enforce-version=latest \
  pod-security.kubernetes.io/warn=restricted \
  pod-security.kubernetes.io/audit=restricted --overwrite >/dev/null 2>&1
...
metadata:
  name: payments-api
  namespace: payments
  labels: { app: payments-api, plane: control, tier: lab }
spec:
  replicas: 1
  ...
  template:
    metadata:
      labels: { app: payments-api, plane: control, tier: lab }
    spec:
      # MUTATED (baseline: a restricted-compliant securityContext here and on the
      # container) — omitted, so admission rejects every Pod under enforce=restricted.
      containers:
        - name: app
          image: busybox:1.36
