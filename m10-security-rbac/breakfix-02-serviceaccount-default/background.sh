...
# Keep timeouts short — Killercoda caps background-script runtime. The RBAC objects
# and the reader workload are applied AFTER this wait (below), so the CrashLooping
# reader never stalls the healthy-fleet readiness check.
...
# >>> breakfix-02 mutation: layer an endpoint-discovery reader, route-watcher, in the
#     call-routing namespace. Here the RBAC is CORRECT: a ServiceAccount (route-watcher),
#     a Role granting get/list/watch on endpoints, and a RoleBinding tying them together.
#     The bug is on the Pod: its template omits serviceAccountName, so the Pod runs as the
#     namespace 'default' SA — which has no binding — instead of route-watcher. The reader
#     lists endpoints and is denied with 403 Forbidden naming ".../default" (not
#     route-watcher). The permission is right; the caller isn't who you think.
#     Fix = set serviceAccountName: route-watcher on the Deployment's Pod template.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
kind: ServiceAccount
metadata:
...
metadata:
  name: route-watcher
  namespace: call-routing
  labels: { app: route-watcher, plane: control, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: route-watcher } }
  template:
    metadata:
      labels: { app: route-watcher, plane: control, tier: lab }
    spec:
      # MUTATED (baseline: serviceAccountName: route-watcher) — omitted, so the Pod runs
      # as the call-routing 'default' SA, which has no binding -> 403 Forbidden.
      containers:
        - name: app
            ...
            - |
              ...
              while true; do
                code=$(curl -sS -o /tmp/body -w '%{http_code}' \
                  ...
                  https://kubernetes.default.svc/api/v1/namespaces/$NS/endpoints)
                echo "GET /api/v1/namespaces/$NS/endpoints -> HTTP $code"
                cat /tmp/body; echo
                if [ "$code" != "200" ]; then
                  echo "endpoint discovery FAILED (HTTP $code) — exiting"
                  exit 1
                fi
                sleep 30
              done
