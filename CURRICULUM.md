...
| M16 Kustomize          | ✅     | ✅         | ✅        | 3 shipped (`patch-target-mismatch`, `generator-name-mismatch`, `commonlabels-immutable-selector`) | GitOps track. Client-side Kustomize (`kubectl apply -k`, no install) — an `edge-relay` base (deployment + service + configMapGenerator) + a `regional-affinity` component + `lab`/`prod` overlays. Render/patch differential (patch selector matches nothing → silently dropped / configMapGenerator hash-suffix reference stale → dangling ref / commonLabels mutate an immutable Deployment selector → apply rejected). Lowest live-smoke risk of the wave (kustomize is client-side). LLM judge PASS |
| M17 Helm               | ✅     | ✅         | ✅        | 3 shipped (`values-key-ignored`, `bad-upgrade-rollback`, `render-required-value`) | GitOps track. Installs real Helm v3.21.2 + a local `voicemail` chart (templates + values.yaml). Chart→values→render→release differential (values key doesn't match the template path → silently ignored / broken `helm upgrade` → `helm rollback` / required value missing → render fails). ⚠ Live-smoke: pinned helm URL resolves + chart renders. LLM judge PASS |
