...
#     a versioned *release*. This scenario installs the release with a wrong-key
#     values override baked in (see the breakfix-01 mutation below). Pinned to
#     the current Helm 3 line.
# ===========================================================================
...
# >>> breakfix-01 mutation: install with a values override that sets the WRONG
#     key. The operator meant to run 3 replicas but wrote `replicas: 3`; the
#     chart's key is `replicaCount`. Helm keeps the unknown key (it does not
#     validate overrides against the chart) but the template never reads it, so
#     the release comes up at the chart default of 1 replica.
cat > /root/voicemail-values.yaml <<'VALS_EOF'
# Intended: run 3 replicas of voicemail.
replicas: 3                       # MUTATED (baseline: replicaCount: 2) — wrong key
config:
  sipRealm: polyphone.example
VALS_EOF
helm install voicemail "$CHART" \
  --namespace app-services \
  -f /root/voicemail-values.yaml >/dev/null 2>&1
