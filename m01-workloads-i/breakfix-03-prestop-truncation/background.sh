...
# >>> breakfix-03 mutation: the preStop hook drains in-flight sessions for 15s,
#     but terminationGracePeriodSeconds is 1 — far too short. The grace period
#     bounds preStop + SIGTERM combined, so the kubelet SIGKILLs the container
#     ~1–3s in (after a one-time ~2s extension), truncating the drain. The pod
#     runs fine; the bug only shows on shutdown — every rollout/scale-down drops
#     in-flight sessions. Teaching: size the grace period for the real drain.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    metadata:
      labels: { app: session-broker, plane: media, tier: lab }
    spec:
      terminationGracePeriodSeconds: 1   # MUTATED (baseline: unset → default 30; too short for the 15s preStop drain → SIGKILL mid-drain)
      containers:
        - name: app
          image: nginx:1.25
          ...
          lifecycle:
            preStop:
              exec: { command: ["/bin/sleep", "15"] }   # simulate draining in-flight media sessions
          resources:
...
# Keep timeouts short — Killercoda caps background-script runtime. session-broker
# itself comes up healthy here (the grace-period bug only shows on shutdown), so
# these waits all pass; the short caps are just insurance against the runtime cap.
