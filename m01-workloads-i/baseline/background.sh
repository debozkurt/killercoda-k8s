...
# >>> M01 baseline enhancement: the gold-standard workload. All three probes,
#     graceful shutdown (preStop drain + 30s grace period), consistent port 80,
#     2 replicas. This is the "what good looks like" reference that the M01
#     breakfix scenarios each mutate one piece of.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
      ...
      containers:
        - name: app
          ...
          startupProbe:
            ...
            periodSeconds: 2
            failureThreshold: 30        # up to ~60s to boot before liveness/readiness engage
          readinessProbe:
            httpGet: { path: /, port: http }
            periodSeconds: 5
            failureThreshold: 3
          ...
          lifecycle:
            preStop:
              exec: { command: ["/bin/sleep", "5"] }   # let Endpoints drain before SIGTERM
          resources:
            requests: { cpu: 50m, memory: 64Mi }
            limits:   { cpu: 200m, memory: 128Mi }
---
