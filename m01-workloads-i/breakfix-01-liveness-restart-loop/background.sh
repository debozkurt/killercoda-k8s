...
# >>> breakfix-01 mutation: liveness probe points at /healthz, a path nginx
#     does not serve (it returns 404). Only HTTP 200–399 pass a probe, so the
#     liveness check fails every period and the kubelet kills + restarts a
#     perfectly healthy container — a self-inflicted CrashLoopBackOff. The app
#     logs stay clean; the truth is in `describe pod` (Liveness probe failed →
#     Killing) and `logs --previous`. Teaching: real crash vs probe killing it.
    ...
    spec:
      containers:
        - name: app
          ...
          livenessProbe:
            httpGet: { path: /healthz, port: http }   # MUTATED (baseline: no liveness probe; nginx has no /healthz → 404 → restart loop)
...
# mutated route-engine will flap (liveness keeps killing healthy pods) rather
# than settle Available. Don't block the whole setup waiting on the broken one.
