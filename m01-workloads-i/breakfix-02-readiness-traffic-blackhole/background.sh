...
# >>> breakfix-02 mutation: readiness probe points at port 8080, but the
#     container serves on 80 — so the probe gets connection-refused every
#     period and the Pod's Ready condition never goes true. A failing readiness
#     probe does NOT restart the container (that's liveness); it removes the Pod
#     from the Service's Endpoints. Result: pods Running + 0/1 READY + 0
#     restarts, Service endpoints empty, callers blackholed. Teaching:
#     readiness gates traffic, not lifecycle.
cat <<'EOF' | kubectl apply -f -
apiVersion: v1
    ...
    spec:
      containers:
        - name: app
          ...
          readinessProbe:
            httpGet: { path: /, port: 8080 }   # MUTATED (baseline: no readiness probe; app serves :80, nothing on :8080 → never Ready → pulled from Endpoints)
            periodSeconds: 5
            failureThreshold: 3
          resources:
            requests: { cpu: 25m, memory: 32Mi }
...
# mutated directory Deployment will never become Available (readiness never
# passes — that's the bug). Don't block the whole setup waiting on it.
