...
# Shorter ceilings — Killercoda caps background-script runtime. The mutation widens a webhook's
# scope, not fleet health, so every fleet Pod still becomes Ready.
  ...
  - name: validate.admission-guard.polyphone.example
    ...
    rules:
      - operations: ["CREATE"]
        ...
        resources: ["pods"]
        scope: Namespaced
    # >>> breakfix-03 mutation: the validating webhook's namespaceSelector was widened to {},
    #     which matches EVERY namespace — not just tenant-apps. It now intercepts Pod creates
    #     across the cluster (signaling, etc.) and requires an env label there, even though the
    #     mutating webhook (still scoped to tenant-apps) never injects env outside tenant-apps.
    #     Fix = narrow it back to matchLabels {admission-guard: enabled}.
    namespaceSelector: {}   # MUTATED (baseline: matchLabels: { admission-guard: enabled })
...
# --- A compliant tenant workload deployed WITHOUT an env label. The mutating webhook injects
#     env=tenant on the way in; the validating webhook then sees it and admits the Pod — the
#     mutate-then-validate ordering, end to end.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  labels: { app: tenant-web, plane: tenant, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: tenant-web } }
  template:
    metadata:
      labels: { app: tenant-web, plane: tenant, tier: lab }   # note: no env label authored here
    spec:
      containers:
        - name: app
          image: nginx:1.25
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
EOF
# tenant-web stays healthy: it's in tenant-apps, where the mutating webhook injects env and the
# (now cluster-wide) validating webhook still sees it. The collateral damage is elsewhere.
kubectl wait --for=condition=Available deployment/tenant-web -n tenant-apps --timeout=90s >/dev/null 2>&1

# sip-canary: a canary rollout in the signaling namespace — a namespace the webhook was never
# meant to govern. The widened validating webhook now intercepts its Pods and requires an env
# label; the mutating webhook is still scoped to tenant-apps, so it never injects env here, and
# the Pod is rejected. sip-canary sits at 0/N with the denial on a FailedCreate event.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
  name: sip-canary
  namespace: signaling
  labels: { app: sip-canary, plane: signaling, tier: lab }
spec:
  replicas: 1
  selector: { matchLabels: { app: sip-canary } }
  template:
    metadata:
      labels: { app: sip-canary, plane: signaling, tier: lab }
    spec:
