...
# Wait for the backend to have a ready endpoint BEFORE registering the webhooks — with
# failurePolicy: Fail, a webhook registered against a backend with no endpoints would block the
# very tenant-web Pod we deploy next.
kubectl wait --for=condition=Available deployment/admission-guard -n admission --timeout=150s >/dev/null 2>&1
