...
# >>> breakfix-01 mutation: the webhook backend is scaled to zero. With no ready endpoint and
#     failurePolicy: Fail, the API server treats every Pod-create call it can't complete as a
#     denial, so Pods in tenant-apps are rejected with "no endpoints available for service
#     admission-guard" and sit at 0/N. Fix = scale admission-guard back to 1 so its endpoint
#     returns and admission calls succeed again.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  replicas: 0     # MUTATED (baseline: replicas: 1)
...
# Register the webhooks regardless of backend health — a configuration can point at a down
# backend, which is exactly this scenario. (With replicas: 0 the Deployment reports Available
# immediately, since zero desired means zero required.)
kubectl wait --for=condition=Available deployment/admission-guard -n admission --timeout=30s >/dev/null 2>&1
...
# --- billing-api: a new tenant service deployed into the governed namespace. Its Pods can't be
#     admitted because the webhook backend is down (fail-closed), so it sits at 0/N with the
#     denial on a FailedCreate event on its ReplicaSet — no Pod to describe or log.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
