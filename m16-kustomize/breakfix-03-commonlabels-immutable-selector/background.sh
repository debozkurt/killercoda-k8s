...
#
# In this scenario lab is already deployed and healthy; the prod overlay uses
# `commonLabels`, which injects its label into the Deployment's IMMUTABLE
# spec.selector. Promoting prod over the running lab Deployment is rejected by
# the API server ("field is immutable"). The render succeeds; the APPLY fails.
# See the fenced breakfix-03 mutation below.
# ===========================================================================
    ...
    spec:
      containers:
        - name: app
          ...
          # `edge-relay-config` must match the generator's name for Kustomize
          # to rewrite this reference to the hash-suffixed object it creates.
          envFrom:
...
# >>> breakfix-03 mutation: the prod overlay stamps its tier label with
#     `commonLabels` instead of the modern `labels:` transformer. `commonLabels`
#     defaults to includeSelectors:true — it writes the label into EVERY selector,
#     including the Deployment's spec.selector.matchLabels, which is immutable after
#     creation. lab is already applied (selector {app: edge-relay}); promoting prod
#     tries to change the selector to {app: edge-relay, tier: prod} and the API
#     server rejects it: the Deployment "edge-relay" is invalid: spec.selector:
#     ... field is immutable. Render succeeds; the apply is refused; edge-relay
#     stays on the lab spec. Fix = use `labels:` with includeSelectors:false (stamp
#     metadata, leave the selector alone), or recreate the Deployment if the label
#     truly must be in the selector.
...
commonLabels:            # MUTATED (baseline: labels: [{pairs:{tier:prod}, includeSelectors:false}] — commonLabels writes the label into the immutable selector)
  tier: prod
images:
  - name: nginx
    newTag: "1.27"
...
# --- deploy lab first (healthy: selector {app: edge-relay}), then attempt the
#     prod promotion — REJECTED by the API server (immutable selector). The
#     rejection is suppressed here; the learner reproduces and reads it in step 1.
kubectl apply -k /root/edge-relay/overlays/lab >/dev/null 2>&1
