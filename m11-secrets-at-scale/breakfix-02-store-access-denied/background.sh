...
# ===========================================================================
# M11 secrets-at-scale additions (as in the baseline), EXCEPT the RoleBinding that
# grants the operator read access to the backing store binds the WRONG ServiceAccount
# (see the fence). The operator therefore cannot read secrets-source, sets EVERY
# SecretSync to StoreNotReady, and materializes no target Secrets — so both consumers
# are down at once. A single mis-subjected binding takes the whole pipeline offline.
# ===========================================================================
...
roleRef:
  ...
  name: secret-operator-secrets
subjects:
  # >>> breakfix-02 mutation: this binding grants store-read to the WRONG ServiceAccount.
  #     The operator authenticates as secrets-system/secret-operator, but the subject here
  #     names secret-operator-ro — a ServiceAccount that doesn't exist. So the operator's
  #     identity has no read access in secrets-source; its `kubectl get secret vault-backend`
  #     fails, and it sets every SecretSync to reason=StoreNotReady. No target Secrets are
  #     created, so BOTH consumers (billing-processor, partner-connector) are in
  #     CreateContainerConfigError. The RBAC parses fine and the operator Pod is Running —
  #     the signal is in the syncs' .status and `auth can-i --as=<the operator SA>`.
  #     Fix = correct the subject name back to secret-operator.
  - kind: ServiceAccount
    name: secret-operator-ro          # MUTATED (baseline: secret-operator) — binds a non-existent SA, so the operator gets no store read
    namespace: secrets-system
...
# With the store binding broken, NO target Secret will ever materialize — every sync
# goes StoreNotReady. Give the operator a couple of reconcile passes to record that in
# .status, then start the consumers anyway (they will be stuck, which is the point).
sleep 20
...
# Neither consumer can start — their target Secrets don't exist — so do NOT block on
# rollouts. Give them a moment to reach CreateContainerConfigError so the symptom is
# visible when the learner arrives.
sleep 12
