...
# ===========================================================================
# M11 secrets-at-scale additions (as in the baseline), EXCEPT this scenario stages a
# ROTATION that reached the Secret but not the consumer. This is a deliberate post-start
# change (the documented exception to deploy-broken-from-start — the lesson IS the
# post-start rotation). See the fences: the store holds the NEW (rotated) db-password,
# but billing-processor is started on the OLD value BEFORE the operator syncs, so its
# env freezes the old credential (M03) while the pipeline goes green on the new one.
# ===========================================================================
...
# --- The backing store. db-password holds the ROTATED value.
# >>> breakfix-03 mutation: db-password is the NEW, post-rotation value (rotation, part 1 of 2).
#     Baseline used 'S3cure-prod-4417'; the credential was rotated to 'R0tated-prod-8842'.
#     The operator will sync this NEW value into the db-credentials Secret — the pipeline
#     ends up green on the new value. The staleness is entirely on the consumer side.
kubectl create secret generic vault-backend \
    --from-literal=db-password='R0tated-prod-8842' \
...
# >>> breakfix-03 mutation: stage the pre-rotation state on the consumer (rotation, part 2 of 2).
#     Seed the db-credentials Secret with the OLD value and start billing-processor on it
#     BEFORE the operator runs, so its DB_PASSWORD env var freezes the old credential at
#     container start (M03). When the operator later syncs the store's NEW value into this
#     same Secret, the object updates but the running Pod does not — a green pipeline above
#     a stale consumer. Fix (for the learner) = roll the consumer so it re-reads the Secret.
kubectl create secret generic db-credentials \
    --from-literal=DB_PASSWORD='S3cure-prod-4417' \
    -n provisioning --dry-run=client -o yaml | kubectl apply -f - >/dev/null
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
      ...
      containers:
        - name: app
          ...
          resources:
            ...
            limits:   { cpu: 100m, memory: 64Mi }
EOF
# Let billing-processor come up and FREEZE the old value in its environment before the
# operator gets a chance to sync the new one.
kubectl rollout status deployment/billing-processor -n provisioning --timeout=90s >/dev/null 2>&1
# <<< breakfix-03 mutation ends

# --- The operator (identical to the baseline). -------------------------------
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
# --- The SecretSync objects (identical to the baseline). ---------------------
cat <<'EOF' | kubectl apply -f -
...
# Wait until the operator has (a) materialized the partner-api Secret and (b) OVERWRITTEN
# db-credentials with the NEW store value — so the pipeline is genuinely green on the new
# credential while billing-processor still holds the old one in its (already-frozen) env.
for i in $(seq 1 40); do
  NEWVAL="$(kubectl get secret db-credentials -n provisioning -o jsonpath='{.data.DB_PASSWORD}' 2>/dev/null | base64 -d 2>/dev/null)"
  kubectl get secret partner-api -n media >/dev/null 2>&1 && [ "$NEWVAL" = "R0tated-prod-8842" ] && break
  sleep 3
done

# --- partner-connector (media): started after its Secret exists, so it is consistent —
#     only the db side is stale. (Identical spec to the baseline.)
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
