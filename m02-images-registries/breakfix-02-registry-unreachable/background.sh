...
# >>> breakfix-02 mutation: the image reference names a registry host that does
#     not resolve (registry.polyphone.example — there is no such DNS name). The
#     kubelet DOES try to pull, but containerd can't even resolve the host, so the
#     pull errors with "no such host" and the pod cycles ErrImagePull /
#     ImagePullBackOff. Models a fat-fingered or decommissioned registry host.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: app
          image: registry.polyphone.example/library/nginx:1.25   # MUTATED (baseline: nginx:1.25 from Docker Hub)
...
# mutated account-provisioner will never become Available (registry host won't
# resolve). Don't block the whole setup waiting on the broken one.
