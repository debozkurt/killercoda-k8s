...
# >>> breakfix-01 mutation: imagePullPolicy: Never on a tag the node has never
#     cached (nginx:1.27 — the fleet only ever pulled nginx:1.25). The kubelet
#     refuses to contact a registry, finds nothing cached, and the pod stalls at
#     ErrImageNeverPull. Models a deploy that copied a Never policy from an
#     air-gapped manifest and bumped the tag without pre-loading the image.
cat <<'EOF' | kubectl apply -f -
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: app
          image: nginx:1.27                # MUTATED (baseline: nginx:1.25, which IS cached)
          imagePullPolicy: Never           # MUTATED (baseline: unset → defaults to IfNotPresent)
          ports: [{ containerPort: 80 }]
          resources:
            requests: { cpu: 50m, memory: 64Mi }
            limits:   { cpu: 200m, memory: 128Mi }
...
# mutated metrics-aggregator will never become Available (ErrImageNeverPull).
# Don't block the whole setup waiting on the broken one.
